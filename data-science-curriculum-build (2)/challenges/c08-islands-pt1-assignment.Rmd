...
-   Are there any sources of *real* uncertainty in the percent occupied you calculated?
    -   No, there is a defined and whole number amount of houses and a whole number of those houses are unoccupied.
-   Are there any sources of *erroneous* uncertainty in the percent occupied you calculated?
    -   I may have miscounted the number of unoccupied houses, I might have missed one. I also rounded the percentage of occupied homes to two decimal places.
...
-   Is this sample representative of *all* houses in Helvig? Why or why not?
    -   No, this is only a sample of the first 25 houses in Helvig. There may be a large household in those first 25 houses that would distort the prevalence of a last name.
...
-   Which sample---sequential or random---is more *representative* of all homes Helvig? Why?
    ...
    -   While the number of households doen't change, the location of them does. By taking houses from a greater population sample of the town we have information from houses that are spread apart from each other. Houses that are close together are more likely to have the same last name because they are more likely to be related to each other, but when the houses are further away from each other the individuals in households are less likely to be related to the next household in the sequence so we get a more accurate representation of the last names in Helvig.
...
-   Did you find any highly prevalent names using `df_sample_random` that you *didn't* find in q3 (using `df_sample_seq`)? Write them here.
    -   Thorn and Ibsen appear more than once in the random sample but do not appear in the sequential sample. Another thing I found interesting was that in the random sample, only 8 names appear more than once, but the majority of names in the sequential sample have at least two people with the same last name.
-   Is there any reason that people with the same last name might tend to *live near each other*?
    -   When people move they will move with their family and will often stay nearby other family they have. Additionally, a person may marry another person near them and that person will take their last name, then they may have kids that will also take their last name.
...
-   Note that we used the *random* sample with the bootstrap procedure in this task. Could we use the bootstrap to make a confidence interval using the sequential sample (`df_sample_seq`) that would be representative of all of Helvig? Why or why not?
    -   No
    -   It would not be representative of all of Helvig because there would be a bias towards last names due to the fact that people that come from one family and have the same last name will live near each other

...
- Comment on the width of your CI(s). Would your estimate of $\pi$ be
  ...
  orbit)?
  - (Good enough as a rough estimate?) Yes, the initial estimate is
    pretty close to pi, within 0.01. And the ranges for the CIs are
    enough for a general estimate, a little more than 3 and a little
    less then 3.3, a difference of 0.3 is pretty negligible for rough
    estimates.
  - (Good enough as a precise estimate?) Definitely not, the ranges we
    are given leave a lot room for error if things like 0.001 of an
    angle matter, or even if 0.01 matters. The initial mightttt be a

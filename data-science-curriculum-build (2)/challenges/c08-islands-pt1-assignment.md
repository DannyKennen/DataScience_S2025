...
- Are there any sources of *erroneous* uncertainty in the percent
  ...
  - I may have miscounted the number of unoccupied houses, I might have
    missed one. I also rounded the percentage of occupied homes to two
    decimal places.
...
- Is this sample representative of *all* houses in Helvig? Why or why
  not?
  - No, this is only a sample of the first 25 houses in Helvig. There
    may be a large household in those first 25 houses that would distort
    the prevalence of a last name.
...
- Did you find any highly prevalent names using `df_sample_random` that
  ...
  - Thorn and Ibsen appear more than once in the random sample but do
    not appear in the sequential sample. Another thing I found
    interesting was that in the random sample, only 8 names appear more
    than once, but the majority of names in the sequential sample have
    at least two people with the same last name.
- Is there any reason that people with the same last name might tend to
  *live near each other*?
  - When people move they will move with their family and will often
    stay nearby other family they have. Additionally, a person may marry
    another person near them and that person will take their last name,
    then they may have kids that will also take their last name.
...
## HINT: Remember that you need to use analysis() when operating on split_df
         name_prevalence(analysis(split_df), last = "Carlsen")  
      }
    )
  ) %>% 
...
- Note that we used the *random* sample with the bootstrap procedure in
  ...
  - No
  - It would not be representative of all of Helvig because there would
    be a bias towards last names due to the fact that people that come
    from one family and have the same last name will live near each
    other
...
- Ruler test to identify reaction times.
